...
function addon:CreateModernUI()
    ...
    -- Flecha Inicio/Fin: alterna el mapa entre zona de inicio (giver) y fin (ender)
    local seArrow = CreateFrame("Button", nil, questImgBox)
    seArrow:SetSize(26, 26)
    seArrow:SetPoint("TOPRIGHT", questImgBox, "TOPRIGHT", -6, -6)
    seArrow:SetFrameLevel(questImgBox:GetFrameLevel() + 20)
    seArrow:SetNormalTexture("Interface\\Buttons\\UI-SpellbookIcon-NextPage-Up")
    seArrow:SetPushedTexture("Interface\\Buttons\\UI-SpellbookIcon-NextPage-Down")
    seArrow:SetHighlightTexture("Interface\\Buttons\\UI-Common-MouseHilight")
    seArrow:Hide()
    local seLbl = questImgBox:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")
    seLbl:SetPoint("TOPLEFT", questImgBox, "TOPLEFT", 8, -8)
    seLbl:SetTextColor(1, 0.82, 0)
    seLbl:Hide()
    questImgBox.startEndArrow = seArrow
    questImgBox.startEndLbl = seLbl
    seArrow:SetScript("OnClick", function()
    ...
    seArrow:SetScript("OnEnter", function(self)
        GameTooltip:SetOwner(self, "ANCHOR_LEFT")
        GameTooltip:SetText(IsSpanish() and "Ver inicio / fin" or "Toggle start / end")
        GameTooltip:Show()
    end)
    seArrow:SetScript("OnLeave", function() GameTooltip:Hide() end)
    ...
    function questImgBox:SetQuest(q)
        ...
        if questImgBox.startEndArrow then
            if _hasTwo then
                ...
                questImgBox.startEndLbl:SetText((questImgBox.showEnd and (IsSpanish() and "Fin: " or "End: ") or (IsSpanish() and "Inicio: " or "Start: ")) .. (GetZoneName(mapZone) or ""))
            else
