...
local IMG_COUNT = { Alliance = 212, Horde = 181 }

local function GetQuestTexture(path, zoneId)
    if not path or path == "" or path:find("placeholder") then
        -- Imagen genérica determinista según zona y facción de la guía activa
        if zoneId then
            local sk = _G.SKquests
            local faction = (sk and sk.db and sk.db.currentGuide) or "Alliance"
            local folder = faction == "Horde" and "Images Horde" or "Images Ally"
            local count = IMG_COUNT[faction] or IMG_COUNT.Alliance
            local idx = (zoneId % count) + 1
            return string.format("Interface\\AddOns\\SKquests\\Media\\%s\\imagen_%03d.tga", folder, idx)
        end
...
local function BuildZonesList()
    ...
    for id, q in pairs(SKquests_DetailDB) do
        ...
        if IsQuestEligible(id, q) and q.zoneId and ZoneMap[q.zoneId] then
...
function addon:CreateModernUI()
    ...
    for i = 1, 5 do
        ...
        btn:SetScript("OnClick", function(self)
            ...
            zoneBtn.lbl:SetText(self.zoneName == "Todas" and "Zonas" or self.zoneName)
            zoneMenu:Hide()
            BuildFilteredQuestIds()
            addon:UpdateListRows()
        end)
    ...
    facBtn:SetScript("OnClick", function(self)
        ...
        addon:UpdateListRows()
        addon:RefreshDetail()
        facBtn.lbl:SetText(nextG == "Alliance" and "Alianza" or "Horda")
    end)
    ...
    local questImgTex = questImgBox:CreateTexture(nil, "BACKGROUND")
    questImgTex:SetAllPoints(questImgBox)
    questImgBox.tex = questImgTex
    ...
    npcLbl:SetText("EMPEZAR / ENTREGAR")
    ...
    linkLbl:SetText("ENLACE WOWHEAD")
    ...
    rsTitle:SetText("DETALLES DE QUEST")
    ...
    chainLbl:SetText("CADENA DE QUESTS")
    ...
    cfgTitle:SetText("SKquests - Configuración")
    ...
    themeLbl:SetText("Tema de apariencia:")
    ...
    opLbl:SetText("Opacidad de ventana:")
    ...
    if lockLabel then
        lockLabel:SetText("Bloquear movimiento de ventana")
    ...
    facLbl:SetText("Fórmula de guía actual:")
    ...
    facBtn:SetText(addon.db and addon.db.currentGuide == "Alliance" and "Alianza" or "Horda")
    facBtn:SetScript("OnClick", function(self)
        local currentG = addon.db and addon.db.currentGuide or "Horde"
        local nextG = currentG == "Alliance" and "Horde" or "Alliance"
        addon:SetCurrentGuide(nextG)
        self:SetText(nextG == "Alliance" and "Alianza" or "Horda")
        BuildFilteredQuestIds()
        addon:UpdateListRows()
        addon:RefreshDetail()
    end)
    ...
    abTitle:SetText("Acerca de SKquests")
...
function addon:RefreshList()
    ...
    if activeTab == "quests" then
        MainFrame.hCount:SetText("(" .. #filteredQuestIds .. " misiones encontradas)")
    elseif activeTab == "questlog" then
        local cache = addon.Tracker:GetActiveQuests()
        local count = 0
        for _ in pairs(cache) do count = count + 1 end
        MainFrame.hCount:SetText("(" .. count .. " misiones activas)")
    elseif activeTab == "guide" then
        MainFrame.hCount:SetText("(" .. #filteredGuideSteps .. " pasos)")
    elseif activeTab == "zones" then
        MainFrame.hCount:SetText("(" .. #uniqueZones .. " zonas activas)")
    end
end
...
function addon:RefreshDetail()
    ...
    if activeTab == "quests" then
        ...
        if not selectedQuestId or not q then
            ch.header.title:SetText("Ninguna misión seleccionada")
            ch.header.meta:SetText("Selecciona una misión del panel izquierdo.")
            ch.header.level:SetText("")
            ch.objSec.box.text:SetText("No hay detalles.")
        ...
        if q.name_loc and q.name_loc ~= "" and q.name_loc ~= q.name then
        ...
        local zoneName = GetZoneName(q.zoneId)
        ch.header.meta:SetText("Zona: |cffffffff" .. zoneName .. "|r")

        -- Mostrar Ilustración
        if q.image then
            ch.questImgBox.tex:SetTexture(GetQuestTexture(q.image, q.zoneId))
            ch.questImgBox:Show()
        else
            ch.questImgBox:Hide()
        end
    ...
    elseif activeTab == "questlog" then
        ...
        if q and q.name_loc and q.name_loc ~= "" and q.name_loc ~= q.name then
            titleText = q.name_loc .. " (" .. q.name .. ")"
        end
        ch.header.title:SetText(titleText)
