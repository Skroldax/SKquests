...
-- JIT parsers for memory optimization
function SKQ_JIT_Unpack(db_table, id)
    if not db_table or not id then return nil end
    local val = db_table[id]
...
function SKQ_ParsePfCoords(coords_str)
    if not coords_str then return {} end
    local t = {}
    for coord in string.gmatch(coords_str, "([^;]+)") do
        local x, y, z, w = string.match(coord, "^([0-9.%-]+),([0-9.%-]+),?([0-9.%-]*),?([0-9.%-]*)$")
        if x and y then
            local entry = { tonumber(x), tonumber(y) }
            if z and z ~= "" then table.insert(entry, tonumber(z)) end
            if w and w ~= "" then table.insert(entry, tonumber(w)) end
...
function SKQ_ParseSpawns(spawns_str)
    if not spawns_str then return {} end
    local t = {}
    for zone_block in string.gmatch(spawns_str, "([^|]+)") do
        local zone_id, coords_str = string.match(zone_block, "^(%d+):(.*)$")
        if zone_id and coords_str then
            t[tonumber(zone_id)] = SKQ_ParsePfCoords(coords_str)
        end
    end
    return t
end
...
local HBD = LibStub and (LibStub("HereBeDragonsQuestie-2.0", true) or LibStub("HereBeDragons-2.0", true))

local HBDPins = LibStub and (LibStub("HereBeDragonsQuestie-Pins-2.0", true) or LibStub("HereBeDragons-Pins-2.0", true))
...
local function GetQuestLoc(id)
    ...
    if IsSpanish() and pfDB and pfDB["quests"] and pfDB["quests"]["esES"] then
        ...
        return SKQ_JIT_Unpack(pfDB["quests"]["esES"], id)
...
local function GetUnitData(npcId)
    ...
    local u = pfDB["units"]["data"] and SKQ_JIT_Unpack(pfDB["units"]["data"], npcId)

    if u and u.coords_str and not u.coords then u.coords = SKQ_ParsePfCoords(u.coords_str) end

    if u and u.coords and #u.coords > 0 then return u end

    return nil -- data-ascension NPC DB removed
...
local function GetObjectData(objId)
    ...
    return pfDB["objects"]["data"] and SKQ_JIT_Unpack(pfDB["objects"]["data"], objId)
...
local function UnitDisplayName(npcId)
    ...
    local nm = (IsSpanish() and u and u["esES"] and SKQ_JIT_Unpack(u["esES"], npcId))

        or (u and u["enUS"] and SKQ_JIT_Unpack(u["enUS"], npcId))

    if nm then return nm end

    local sd = SKquests_SpawnData and SKquests_SpawnData.npcs and SKQ_JIT_Unpack(SKquests_SpawnData.npcs, npcId)
...
local function ObjectDisplayName(objId)
    ...
    local nm = (IsSpanish() and o and o["esES"] and SKQ_JIT_Unpack(o["esES"], objId))

        or (o and o["enUS"] and SKQ_JIT_Unpack(o["enUS"], objId))

    if nm then return nm end

    local sd = SKquests_SpawnData and SKquests_SpawnData.objects and SKQ_JIT_Unpack(SKquests_SpawnData.objects, objId)
...
local function GetZoneName(zoneId)
    ...
    if SKquests_Localization and SKquests_Localization.currentLanguage == "esES"
       ...
       and pfDB and pfDB["zones"] and SKQ_JIT_Unpack(pfDB["zones"], "esES") and SKQ_JIT_Unpack(pfDB["zones"], "esES")[zoneId] then

        return SKQ_JIT_Unpack(pfDB["zones"]["esES"], zoneId)
    ...
    if pfDB and pfDB["zones"] then
        ...
        local nm = (SKQ_JIT_Unpack(pfDB["zones"], "enUS") or {})[zoneId]
...
-- Inyección ÃƒÅ¡NICA de BronzebeardQuestChains como quests custom (nombres limpios,
...
local EXPANSION_EXCLUDED_ZONES = {
    ...
    [3430]=true, [3433]=true, -- Eversong Woods / Ghostlands (zona inicial Sangre Ãƒâ€°lfica TBC)
...
local function IsQuestEligible(id, q)
    ...
    local descVal = q.desc or (SKQ_GetQuestText and SKQ_GetQuestText(q.id, "desc"))
    if descVal == "No information available" then return false end
    ...
    local l4 = tonumber(q.reqLevel) or 0
      ...
      if (l1 > 80) or (l2 > 80) or (l3 > 80) or (l4 > 80) then return false end

      local pFac = UnitFactionGroup("player")

      if pFac and GetQuestFaction then

          local qFac = GetQuestFaction(id, q)

          if qFac and qFac ~= "Both" and qFac ~= pFac then return false end
...
GetQuestFaction = function(id, q)
    ...
    if pfDB and pfDB["quests"] and pfDB["quests"]["data"] then
        ...
        local pq = SKQ_JIT_Unpack(pfDB["quests"]["data"], id)
...
local function BuildGuideChapters()
    ...
    for i, step in ipairs(guide) do
        ...
        -- Parse title like "1-5 Northshire Valley Ã¢â‚¬â€ Circuit 1"

        -- Also supports "1-5 Northshire Valley * Circuit 1" or hyphen

        local zoneMatch = rawTitle:match("^%d+%-%d+%s+(.-)%s*[-*~:]%s*Circui[to]+")

        if not zoneMatch then zoneMatch = rawTitle:match("^(.-)%s*[-*~:]%s*Circui[to]+") end
    ...
    if currentTitle ~= "" then
      ...
      if not addon._hasSyncedChapter and addon.db and addon.db.currentStep then

          local syncStep = addon.db.currentStep

          for j, ch in ipairs(guideChapters) do

              if syncStep >= ch.startIndex and syncStep <= ch.endIndex then selectedGuideChapter = j; break end

          end

          addon._hasSyncedChapter = true
...
local function LayoutDetailSections(ch)
    ...
    if ch.rewardSec:IsShown() then

        ch.linkSec:SetPoint("TOPLEFT", ch.rewardSec, "BOTTOMLEFT", 0, -10)

    elseif ch.npcSec:IsShown() then

        ch.linkSec:SetPoint("TOPLEFT", ch.npcSec, "BOTTOMLEFT", 0, -10)

    elseif ch.descSec:IsShown() then

        ch.linkSec:SetPoint("TOPLEFT", ch.descSec, "BOTTOMLEFT", 0, -10)

    else

        ch.linkSec:SetPoint("TOPLEFT", ch.objSec, "BOTTOMLEFT", 0, -10)
...
--  XP APPRAISER Ã¢â‚¬â€ controles dentro del panel de Ajustes
...
function addon:CreateModernUI()
    ...
    -- ---- CONTROL DE REDIMENSIÃƒâ€œN (CORNER HANDLES) ----
    ...
    --  FILTRO DE RANGO DE NIVEL (min/max) Ã¢â‚¬â€ solo pestaña Quests
    ...
    -- Crear los row buttons reutilizables Ã¢â‚¬â€ anclados al listScroll directamente
            ...
            if data.title and GetQuestLogTitle then
                ...
                if t ~= data.title then
                    ...
                    for i = 1, n do
                        ...
                        local ti, _, _, _, isHeader = GetQuestLogTitle(i)
            ...
            if button == "RightButton" then
                ...
                if activeTab == "questlog" and self.itemId then
                    ...
                    StaticPopupDialogs["SKQUESTS_ABANDON"].text =
                        ...
                        (IsSpanish() and "Ã‚Â¿Abandonar la misión \"%s\"*" or "Abandon quest \"%s\"?")
    ...
    function addon:RelayoutZonesMap()
        ...
        if ZonesMapPanel.currentMapMode == "continent" then
            ...
            -- Find out exactly what zoneId the custom map is rendering!
    if ZonesMapPanel.selectedZone then
        print("|cffff0000SKquests Debug:|r Custom Map is rendering zoneId: " .. tostring(ZonesMapPanel.selectedZone))
    end
    ...
    function addon:CollectSpawnCoords(kind, id, zoneId)
        ...
        if kind == "object" then
            ...
            local sd = SKquests_SpawnData and SKquests_SpawnData.objects and SKQ_JIT_Unpack(SKquests_SpawnData.objects, id)
            ...
            if sd and sd.spawns then
                    ...
                    if type(sp) == "string" then sd.spawns[z] = SKQ_ParsePfCoords(sp); sp = sd.spawns[z] end
            ...
            local sd = SKquests_SpawnData and SKquests_SpawnData.npcs and SKQ_JIT_Unpack(SKquests_SpawnData.npcs, id)
            if sd and sd.spawns_str and not sd.spawns then sd.spawns = SKQ_ParseSpawns(sd.spawns_str) end
        ...
        if ZonesMapPanel.currentMapMode == "continent" then
            ...
            if folder then
                ...
                if not usedMap then
                    ...
                    local zd = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], ZonesMapPanel.currentSelectedZoneId)
            ...
            for _, entry in ipairs(quests) do
                ...
                if coordsList and #coordsList > 0 then
                    ...
                    if cz and cz ~= selZone then
                        ...
                        local czData = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], cz)
    ...
    --  PANEL DE SELECCIÃƒâ€œN DE GUÍAS (rejilla de tarjetas, estilo Zonas)
    ...
    function questImgBox:SetQuest(q)
        ...
        if folder then
            ...
            if not usedMap then
                ...
                local zd = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], mapZone)
        ...
        if usedMap then
            ...
            local originalZData = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], originalMapZone)
            ...
            local function PlaceDot(x, y, z, r, g, b, label, sub, category)
                ...
                if customMap and z and z ~= mapZone then
                    ...
                    local parentData = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], mapZone)
                ...
                elseif not customMap and z and z ~= mapZone then
                    ...
                    local czData = pfDB and pfDB["zones"] and pfDB["zones"]["data"] and SKQ_JIT_Unpack(pfDB["zones"]["data"], z)
    ...
    objSec.box = objBox
      ...
      -- Ã¢â€â‚¬Ã¢â€â‚¬ Menú de canal: Say / Party / Guild Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
          ...
          cBtn:SetScript("OnClick", function()
              ...
              if sb and ((sb.shareLines and #sb.shareLines > 0) or sb.isComplete) then
                  ...
                  if sb.shareLines and #sb.shareLines > 0 then
                      ...
                      prog = IsSpanish() and "Ã‚Â¡Completada! Lista para entregar." or "Completed! Ready to turn in."
    ...
    do
        ...
        linkSec:SetPoint("TOPLEFT", dChild.npcSec, "BOTTOMLEFT", 0, -10)

        linkSec:SetPoint("RIGHT", dChild, "RIGHT", -16, 0)

        linkSec:SetHeight(40)

        dChild.linkSec = linkSec



        local linkLbl = linkSec:CreateFontString(nil, "OVERLAY", "GameFontHighlightSmall")

        linkLbl:SetPoint("TOPLEFT", 0, 0)

        linkLbl:SetText(L("WOWHEAD_LINK"))

        linkLbl:SetTextColor(1, 0.82, 0)

        linkSec.lbl = linkLbl



        local linkBox = CreateFrame("EditBox", nil, linkSec, "InputBoxTemplate")

        linkBox:SetSize(250, 20)

        linkBox:SetPoint("TOPLEFT", linkLbl, "BOTTOMLEFT", 6, -4)
    ...
    if t == "dragonflight" then return L("THEME_ASCENSION") end
    ...
    elseif tKey == "dragonflight" then tName = L("THEME_ASCENSION")
...
function addon:RefreshDetail()
        ...
        local q = SKquests_DetailDB[questKey]
          ...
          if SKquests and SKquests.Collector then
            ...
            if q then
              ...
              -- Quest not in static DB Ã¢â‚¬â€ use 100% collected data (custom quest)
    ...
    ch.linkSec.box:SetText("")
        ...
        -- Mostrar títulos bilingÃƒÂ¼es: Español (Inglés)
        ...
        local descText = PfText((locD and locD.D) or q.desc or (SKQ_GetQuestText and SKQ_GetQuestText(q.id, "desc"))) or ""
    ...
    ch.linkSec.box:SetText("")
        ...
        ch.descSec.text:SetText(dbDesc or logDesc or (q and PfText(q.desc or (SKQ_GetQuestText and SKQ_GetQuestText(q.id, "desc")))) or L("NO_INFORMATION"))
    ...
    if q and SKquests_Marks and SKquests_Marks[q.id] then rwParts[#rwParts + 1] = "|cff66ccff" .. SKquests_Marks[q.id] .. " " .. (IsSpanish() and "Marcas de Ascension" or "Marks of Ascension") .. "|r" end
...
function addon:SwitchTab(tabId)
    ...
    if tabId == "guide" then
        ...
        if not addon.selectedGuideKey or not addon.db or not addon.db.currentGuide then

            if GuideCardsPanel then GuideCardsPanel:Show() end

        elseif GuideLockPanel and GuideLockPanel.isLocked then
            ...
            if RightSidebarShown then RightSidebar:Show() end

            if DetailPanel.child then DetailPanel:Show() end
    ...
    elseif tabId == "about" then
        ...
        if tabId == "quests" or tabId == "questlog" then
            ...
            ListPanel.scroll:SetPoint("BOTTOMRIGHT", ListPanel, "BOTTOMRIGHT", -30, 4)
...
local function SKQ_GetNpcCoordInZone(npcId, targetZoneId)
    ...
    for _, c in ipairs(u.coords) do
        ...
        if c[3] == targetZoneId then return c[1], c[2], c[3] end
    ...
    for _, c in ipairs(u.coords) do
        ...
        if czData and czData[1] == targetZoneId and czData[2] and czData[4] then
            ...
            return zx + (c[1] * zw / 100), zy + (c[2] * zh / 100), targetZoneId
    ...
    -- 3. El NPC esta en la zona padre de targetZoneId -> proyectar padre->hijo

    -- (Ej: target=Valle de los Retos, NPC=Durotar)

    local targetData = zonesData and zonesData[targetZoneId]

    if targetData and targetData[1] and targetData[1] ~= targetZoneId and targetData[2] and targetData[4] then

        local parent = targetData[1]
        ...
        for _, c in ipairs(u.coords) do
            ...
            if c[3] == parent then
                ...
                return (c[1] - zx) * 100 / zw, (c[2] - zy) * 100 / zh, targetZoneId
    ...
    -- 4. Fallback: primera coordenada (diferente zona)

    if u.coords[1] then return u.coords[1][1], u.coords[1][2], u.coords[1][3] end
    ...
    if SKquests_DetailDB then
        ...
        for questId, q in pairs(SKquests_DetailDB) do
            ...
            if q.zoneId == zoneId then
                ...
                if IsQuestEligible(questId, q) then

                    local qFac = GetQuestFaction(questId, q)

                    if not qFac or qFac == "Both" or qFac == playerFaction then
...
local function SKQ_GetZoneQuestPins(zoneId)
    ...
    if active then
        ...
        for _, entry in pairs(active) do
            ...
            if not seenQuests[entry.id] then

                local status = entry.isComplete and "complete" or "active"

                local q = SKquests_DetailDB and SKquests_DetailDB[entry.id]

                if q then

                    local npcId, npcType = q.enderId or q.giverId, q.enderType or q.giverType

                    if npcId and npcType ~= "GO" then

                        local x, y = SKQ_GetNpcCoordInZone(npcId, zoneId)

                        if x and y then

                            table.insert(out, { questId = entry.id, npcId = npcId, status = status, q = q })
...
local function SKQ_InitMapIdCaches()
    ...
    local nameToMapId = {}
        ...
        if HBD and HBD.GetLocalizedMap then
        for i = 1, 2000 do
...
local skq_MapInfoToZoneId
local function SKQ_InitZoneCaches()
    if skq_MapInfoToZoneId then return end
    skq_MapInfoToZoneId = {}
    if pfDB and pfDB["zones"] then
        for _, locTable in pairs(pfDB["zones"]) do
            for id, nm in pairs(locTable) do
                if type(nm) == "string" and nm ~= "" then
                    skq_MapInfoToZoneId[SKQ_NormZoneName(nm)] = id
                end
            end
        end
    end
...
SKQ_RefreshWorldMapPins = function()
    ...
    if not skq_InternalMapToZoneId then
        skq_InternalMapToZoneId = {}
        for id, name in pairs(ZoneMapFolder) do
            skq_InternalMapToZoneId[name] = id
            skq_InternalMapToZoneId[string.lower(name)] = id
        end
    end
...
function addon:RefreshMiniTracker()
    ...
    for _, item in ipairs(sortedQuests) do
        ...
        if entry then
            ...
            -- Crear o recuperar botón de expandir/colapsar (Ã¢â€“Âº / Ã¢â€“Â¼)
...
local HBD_Arrow = LibStub and (LibStub("HereBeDragonsQuestie-2.0", true) or LibStub("HereBeDragons-2.0", true))
...
ArrowFrame:SetScript("OnUpdate", function(self, elapsed)
    if not HBD_Arrow then return end
